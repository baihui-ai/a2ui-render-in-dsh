...
会话里出现表单卡时，右缘停靠一个 dsh 风格的面板开关（角标=待填数，轻微脉冲提醒），点开是「会话导航」抽屉。**任务**页签分组列出「待提交任务」（每卡已填计数如 已填 2/6、已填内容预览、日期时间、可标记「无需填写」）与「已提交任务」（✓ 提交时间）；**全部**页签完整列出会话中每条用户消息（不省略、可换行、带年月日时间），点击任意行滚动定位并高亮，老消息自动触发「加载更早」重试，表单挂在对应消息下方实时标注状态，抽屉打开期间每 5 秒自动同步最新消息。草稿边填边存、刷新不丢；已提交状态即使清了浏览器缓存也能从会话记录重建。
